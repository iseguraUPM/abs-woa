...
final class TribeStrategyBehaviour extends SimpleBehaviour implements UnitStatusHanlder {
    ...
    public void onStartingUnitCreation(AID unitAID) {
        ...
        agent.log(Level.FINE, unitAID.getLocalName() + " started creating a unit");
    }
    ...
    public void onFinishedBuilding(AID unitAID, String buildingType) {
        unitFinishedJob(builders, unitAID);
        switch (buildingType) {
            case WoaDefinitions.TOWN_HALL:
                builtTownHalls++;
                break;
    ...
    protected void unitFinishedJob(Collection<ProffesionalUnit> workers, AID unitAID) {
        if (workers.isEmpty()) {
            return;
        }
    ...
    private void assignJobs() {
        ...
        assignStrategies(miners, StrategyFactory
                .envelopExploitResourceStrategy(Strategy.HIGH_PRIORITY
                        , WoaDefinitions.ORE));
